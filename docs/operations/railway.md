...
#### GitHub App access 복구

같은 owner/repository 이름으로 public history를 새로 시작해도 GitHub repository ID는 달라진다.
Railway App installation이 이전 repository ID만 선택한 상태에서는 branch를 명시한 source 연결이
보이더라도 push webhook과 native autodeploy가 동작하지 않을 수 있다. 다음 순서로 복구한다.

1. GitHub Railway App installation에서 현재 public repository를 선택하고 저장한다. Pending
   permission update도 같은 화면에서 승인한다.
2. Railway permission cache가 갱신되도록 몇 분 기다린 뒤 Railway account를 다시 인증한다.
3. Railway project의 **Add -> GitHub Repository -> Refresh**로 repository 목록을 새로 고친다.
4. 정상 production deployment가 계속 실행 중이고 새 `main` push가 없는 시점에 service source를
   disconnect/reconnect한다. GitHub App 가시성을 우회하는 explicit branch 강제 연결로 성공을
   꾸미지 않는다.
5. Source가 현재 public repository, `main`, repository root와 `Wait for CI=true`를 가리키는지
   다시 읽는다.
6. 의미 있는 pull request를 merge해 `backend/**`, `contracts/**`, `Dockerfile`, `.dockerignore` 또는
   `railway.json`의 configured watch path를 바꾸는 `main` push를 만들고 네 GitHub check 이후 exact
   SHA deployment가 `SUCCESS`가 되는지 확인한다. Empty commit과 docs-only push는 증거가 아니다.
7. Railway native `deployment_status=success`가 production edge smoke를 시작했는지 확인한다.

CLI `Unauthorized`가 한 번의 Railway 재인증 뒤에도 계속되면 authenticated Railway UI의 repository
목록과 reconnect 결과를 함께 확인한다. App을 제거했다가 다시 설치하기 전에는 현재 live deployment와
source 설정을 기록하고, 대상 repository selection과 Railway project member의 contributor access를
다시 확인한다. Source reconnect나 수동 최신-commit 배포만 성공한 것은 webhook 경로의 증거가 아니다.
